...
public class PointSET {
    ...
    public Iterable<Point2D> range(RectHV rect) {
        ...
        Point2D bottomLeftPoint = new Point2D(rect.xmin(), rect.ymin());
        Point2D topRightPoint = new Point2D(rect.xmax(), rect.ymax());
        ...
        for (Point2D point: pointBinaryTree) {
            if(point.x() >= bottomLeftPoint.x()
                ...
                && point.x() <= topRightPoint.x()
                && point.y() <= topRightPoint.y()) {
